...
class Coupled(Decoupled):
    ...
    def solvu(self, i):
        ...
        khat = (self._omega ** 2) + 2.0 * self._damp_tot * self._omega * self.gamma / (self.beta * delt) + d1 / (self.beta * (delt ** 2))
        ...
        if i == 1:
            ...
            self.u2 = deltu
            self.udot2 = deltudot
            self.udotdot2 = (-(self.L1 / self.M1) * self.acc22 - 2.0 * self._damp_tot * self._omega * self.udot2 - (self._omega ** 2) * self.u2) / d1
        else:
    ...
    def c_slideacc(self, i):
        # update sliding acceleration based on calc'd response
        if self._slide:
            self.sdotdot2 = -self.a_in[i - 1] * self.gCOS * self.scale_factor - self.k_y(self.s[i-2]) * self.normalf2 / self.mass - self.L1 * self.udotdot2 / self.mass + self.gSIN

        # calc. base force based on a_resp calc
        self.basef = -self.mass * self.a_in[i - 1] * self.gCOS * self.scale_factor - self.L1 * self.udotdot2 + self.mass * self.gSIN
