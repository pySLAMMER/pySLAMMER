...
import pytest
from verification.comparisons import ComparisonEngine, ConfigManager, VerificationSummary
from verification.data_loader import DataManager
from verification.schemas import VerificationData
...
class TestVerificationFramework:
    ...
    @pytest.fixture
    def sample_analysis_records(self, reference_data):
        """Fixture providing a small sample of analysis records."""
        return reference_data.analyses[:5]  # First 5 analyses for fast execution
...
class TestDataManagement(TestVerificationFramework):
    ...
    def test_data_filtering(self, data_manager, reference_data):
        ...
        rigid_analyses = data_manager.filter_analyses(reference_data, methods=["rigid"])
        assert all(analysis.analysis.method == "rigid" for analysis in rigid_analyses)
        assert len(rigid_analyses) > 0

        decoupled_analyses = data_manager.filter_analyses(
            reference_data, methods=["decoupled"]
        )
        assert all(analysis.analysis.method == "decoupled" for analysis in decoupled_analyses)

        # Test combined filtering
        combined_analyses = data_manager.filter_analyses(
            reference_data, methods=["rigid", "decoupled"]
        )
        assert len(combined_analyses) == len(rigid_analyses) + len(decoupled_analyses)

    def test_cache_key_generation(self, data_manager, sample_analysis_records):
        """Test cache key generation is deterministic."""
        analysis_record = sample_analysis_records[0]

        key1 = data_manager.generate_cache_key(analysis_record, "1.0.0")
        key2 = data_manager.generate_cache_key(analysis_record, "1.0.0")
        ...
        key3 = data_manager.generate_cache_key(analysis_record, "1.0.1")
        assert key1 != key3
...
class TestConfigurationManagement(TestVerificationFramework):
    ...
    def test_value_dependent_tolerances(self, config_manager):
        ...
        normal_tolerance = config_manager.get_tolerance(
            "rigid", displacement_value=10.0
        )
...
class TestStatisticalComparison(TestVerificationFramework):
    ...
    def test_individual_comparison(self, comparison_engine):
        ...
        assert (
            abs(result.absolute_error - 0.2) < 1e-10
    ...
    def test_small_displacement_handling(self, comparison_engine):
        ...
        )  # Should pass due to small displacement tolerance
    ...
    def test_group_analysis(self, comparison_engine):
        ...
        group_result = comparison_engine.analyze_group(
            individual_results, "rigid", "normal"
        )
        ...
        assert (
            0.99 < group_result.lin_regression_slope < 1.05
        )  # Should be close to 1.01
        assert (
            group_result.lin_regression_r_squared > 0.95
    ...
    def test_verification_summary_generation(self, comparison_engine):
        ...
        for i in range(3):
            result = comparison_engine.compare_individual_test(
                f"rigid_{i}", "rigid", "normal", float(i + 1), float(i + 1) * 1.01
            )
            individual_results.append(result)
        ...
        for i in range(2):
            result = comparison_engine.compare_individual_test(
                ...
                "normal",
                float(i + 10),
                float(i + 10) * 1.20,  # 20% error - should fail
            )
            individual_results.append(result)
