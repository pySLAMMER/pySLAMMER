...
for ky in kys:
    for motion, hist in histories.items():
        ...
        output[run] = {"motion":motion, "ky":ky, "k_max":np.max(histories[motion].accel), "d_max": result.max_sliding_disp, "dt":result.dt, "disp": result
        .sliding_disp}
        run += 1
```
