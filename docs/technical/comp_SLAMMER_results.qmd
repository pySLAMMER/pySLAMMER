...
pyslammer_path = project_root / "tests" / "verification_data" / "results" / "pyslammer_0.2.2_results.json.gz"
...
The equivalence check against the criteria in @tbl-equiv is implemented through automated pre-release testing in pySLAMMER. All three methods (rigid, decoupled, and coupled) passed all equivalence criteria. The full verification report for the current release is available in the [pySLAMMER repository](https://github.com/pySLAMMER/pySLAMMER/blob/main/tests/verification_data/results/verification_report_v0.2.2.md).
